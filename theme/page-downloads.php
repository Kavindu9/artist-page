...
$downloadSections = new WP_Query( $args );
...
<article class="py-20 lg:py-36">
    <main class="container px-4 sm:px-10 max-w-screen-xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-10 sm:gap-y-2 mb-24">
        ...
        <?php } else { ?>
        <h1 class="page-title font-body text-sm font-normal  text-smoke-white-900 uppercase tracking-wider"><?php the_title(); ?></h1>
        <h3 class="page-headline col-span-1 sm:col-span-3 text-5xl sm:text-7xl font-title font-extralight text-paradise-pink-500 max-w-sm mb-2 lg:mb-4">
            <?php echo $headline; ?>
        </h3>
...
if ( $downloadSections->have_posts() ) { ?>


            <main class="download-sections space-y-12 md:space-y-20">

                <?php while ( $downloadSections->have_posts() ) {
		$downloadSections->the_post();
		// do something
        get_template_part('partials/section', 'download'); 
	} ?>
